...
import { useListarPacientesPacientesGet } from "../../generated/api/pacientes/pacientes";
...
function PacientesPage() {
  const { data: patients = [], isLoading } = useListarPacientesPacientesGet();
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="rounded-[18px] border border-border bg-card p-5 flex items-center gap-4">
          ...
          <div>
            <div className="font-heading text-[34px] font-bold text-foreground">
              {patients.filter((p) => p.sexo === 1).length}
            </div>
        ...
        <div className="rounded-[18px] border border-border bg-card p-5 flex items-center gap-4">
          ...
          <div>
            <div className="font-heading text-[34px] font-bold text-foreground">
              {patients.filter((p) => p.sexo === 0).length}
            </div>
              ...
              <TableBody>
                {patients.map((patient) => (
                  <TableRow key={patient.id}>
                    ...
                    <TableCell>{patient.nome ?? "Sem nome"}</TableCell>
                    <TableCell>{patient.idade} anos</TableCell>
                    <TableCell>
                      <Badge variant={patient.sexo === 1 ? "default" : "secondary"}>
                        {patient.sexo === 1 ? "M" : "F"}
                      </Badge>
                    </TableCell>
